...
class GraphEdgeAttenNetwork(BaseNetwork):
    def __init__(self, num_heads, dim_node, dim_edge, dim_atten, aggr= 'max', use_bn=False,
        ...
        self.dim_node=dim_node
        self.dim_edge=dim_edge
        self.index_get = Gen_Index(aggr=aggr, flow=flow) #node_dim=dim_node,
        self.index_aggr = Aggre_Index(aggr=aggr,node_dim=dim_node, flow=flow)
    ...
    def forward(self, x, edge_feature, edge_index):
        ...
        xx = self.index_aggr(xx, edge_index, dim_size = x.shape[0],dim=-2)
...
class GraphEdgeAttenNetworkLayers(torch.nn.Module):
    ...
    def __init__(self, dim_node, dim_edge, dim_atten, num_layers, num_heads=8, aggr= 'max',
    ...
    def forward(self, node_feature, edge_feature, edges_indices, obj_center, batch_ids):
        ...
        if obj_center is not None:
            ...
            insnum = node_feature.shape[0]
            batch_ids = torch.zeros(insnum)
            ...
            idx_i = torch.where(batch_ids == 0)[0]
