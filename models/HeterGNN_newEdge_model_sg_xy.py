...
comparative_label = [8,9,10,11,12,13]
...
class get_model(nn.Module):
    def __init__(self):
        ...
        if NOWEIGHT==False:
            self.gnn_l = TypeGraphEncoder(ndim=512, nlayer=3)
        ...
        if NOWEIGHT == True:
            self.gnn_l = GraphEncoder(ndim=512, nlayer=3)
            self.gnn_s = GraphEncoder(ndim=512, nlayer=3)
            self.gnn_p = GraphEncoder(ndim=512, nlayer=3)
            self.gnn_comp = GraphEncoder(ndim=512, nlayer=3)

        self.s_mlp = MLP(mlp=[256, 512])
        self.p_mlp = MLP(mlp=[256, 512])
        self.c_mlp = MLP(mlp=[256, 512])
        self.node_mlp = MLP(mlp=[512*3, 512*2, 512])
        self.node_Linear_l = nn.Linear(512, 512)
        self.node_Linear_s = nn.Linear(512, 512)
        self.node_Linear_p = nn.Linear(512, 512)
        self.node_Linear_c = nn.Linear(512, 512)

        self.node_classifer = MLP(mlp=[512, 256, 256, 160])
        self.edge_classifer_l= MLP(mlp=[512,256, 128, 2])
    ...
    def forward(self, obj_codes, pred_codes, types_w, pc_geom_info): #obj_codes(Nn,C_konw) pred_codes (Ne, C)
        insnum = obj_codes.shape[0]
        edgenum = pred_codes.shape[0]
        types_w, multiW_Gated, type_output, _ = self.type_classification(obj_codes, pred_codes)  # Ne,3   Ne,256
        link_w, support_w, p_w, comp_w  = types_w #nouse
        ...
        obj_codes_l = self.node_Linear_l(obj_codes)
        obj_codes_s = self.node_Linear_s(obj_codes)
        obj_codes_p = self.node_Linear_p(obj_codes)
        obj_codes_c = self.node_Linear_c(obj_codes)

        g_l = SceneGraph(x=obj_codes_l, edge_index=edge_index, edge_attr=link_codes, type="link", edge_weight=link_w , remap=remap)  #edge_weight(Ne,) link图
        ...
        self.type = "link"
        node_embed_l, edge_embed_l = self.gnn_l(g_l)
        ...
        node_embed = torch.cat([node_embed_s, node_embed_p, node_embed_c], dim=1)
        node_embed = self.node_mlp(node_embed)
        node_output = self.node_classifer(node_embed)  # ([7, 160])  # ([42, 27])
        node_output = F.softmax(node_output, dim=1)

        logit_l = self.edge_classifer_l(edge_embed_l)  # ([7, 160])  # ([42, 27])
        edge_output_l = F.softmax(logit_l, dim=1)  # (Ne,7)
        ...
        multi_edge_output = [edge_output_l, edge_output_s, edge_output_p, edge_output_c,type_output]
...
class SceneGraphConv(nn.Module):
    ...
    def node_message(self, Hp, insnum):  # s->i  o->j  weight: r_ij
        ...
        wHij_sum = torch.sum(wHij_matrix,dim=1)
