...
def gen_edge_feature(pts):
    num_objects = pts.shape[0]
    #pts_np = pts.cpu().numpy()
    descriptor = torch.zeros([num_objects, 11])
    for i in range(num_objects):
        obj_pointset = pts[i]
        descriptor[i]= gen_descriptor(obj_pointset[:,:3])
    edge_features = descriptor
...
class Gen_edge_descriptor(MessagePassing):#TODO: move to model
    ...
    def forward(self, descriptor, edges_indices): #(No,8) (2,Ne)
