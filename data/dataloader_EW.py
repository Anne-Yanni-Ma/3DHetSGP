...
def calculate_geom_info(obj_points):
    ...
    for idx in range(len(obj_points)):
        ...
        centroid =np.array([x0, y0, z0])
        centroids.append(np.array([x0, y0, z0]))
    info = np.array(bboxes), np.array(lwhV), np.array(centroids)
...
class DataLoader_3DSSG(Dataset):
    def __init__(self, training=True, shuffle=False, norm=False, half=False, per25=False):
        ...
        self.obj_w = torch.Tensor(np.load('/home/ma/myn/2023/SGP_fuseFrame/data/obj_w.npy')).cuda()
    ...
    def __getitem__(self, index):
        ...
        types_output4 = np.load(folder + '/edge_weights.npy')
        #if Predcls: edge_weights.npy   elif: SGCls: edge_weights_sg.npy or none
        if self.norm:
            pc_mat = self.normalize(pc_mat)
        return torch.Tensor(pc_mat), pc_geom_info, torch.Tensor(types_output4), torch.IntTensor(obj_gt), torch.IntTensor(rel_gt)
...
if __name__ == "__main__":
    #dataset3dssg_train = DataLoader_3DSSG(training=True)
    dataset3dssg_test = DataLoader_3DSSG(training=False)

    for scan_id in range(len(dataset3dssg_test.test_list)):
        #print("scan_id: ", scan_id)
        pc_mat,  pc_geom_info, type_link, obj_gt, rel_gt = dataset3dssg_test.__getitem__(scan_id)
